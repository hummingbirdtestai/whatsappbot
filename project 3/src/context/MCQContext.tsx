...
interface MCQContextType {
  ...
  createMockTest: (test: Omit<MockTest, 'id' | 'mcqCount' | 'status' | 'createdAt'> & { whatsappGroups?: string[] }) => void;
...
export const MCQProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sampleMCQs: MCQ[] = [
    ...
    },
    {
      ...
      category: "Technology",
      difficulty: "Medium",
      explanation: "Python is widely used in AI and ML due to its extensive libraries like TensorFlow, PyTorch, and scikit-learn.",
      uploadDate: new Date().toISOString(),
    },
    {
      id: "3",
  ...
  const createMockTest = async (test: Omit<MockTest, 'id' | 'mcqCount' | 'status' | 'createdAt'> & { whatsappGroups?: string[] }) => {
    try {
      ...
      alert('Error creating mock test: ' + (err instanceof Error ? err.message : err));
    }
  };

  const updateMockTestStatus = (id: string, status: string) => {
    setMockTests(prev => prev.map(test => 
      test.id === id ? { ...test, status: status as MockTest['status'] } : test
    ));
  };
