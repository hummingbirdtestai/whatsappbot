...
export const UploadMCQ: React.FC = () => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Failed to parse JSON');
      setUploadStatus('error');
    }
  };
  ...
  // Validate MCQ JSON (options as object, correctAnswer as label)
  const validateMCQs = (data: any): { valid: boolean; error?: string } => {
    if (!Array.isArray(data)) return { valid: false, error: 'JSON must be an array.' };
    for (const [i, mcq] of data.entries()) {
      if (!mcq.question || typeof mcq.options !== 'object' || typeof mcq.correctAnswer !== 'string') {
        return { valid: false, error: `MCQ at index ${i} missing required fields or wrong types.` };
      }
      const optionKeys = Object.keys(mcq.options);
      if (optionKeys.length < 2 || !optionKeys.every(k => OPTION_LABELS.includes(k))) {
        return { valid: false, error: `MCQ at index ${i} must have options labeled A, B, C, D (at least 2).` };
      }
      if (!OPTION_LABELS.includes(mcq.correctAnswer) || !mcq.options[mcq.correctAnswer]) {
        return { valid: false, error: `MCQ at index ${i} has invalid correctAnswer label.` };
      }
    }
    return { valid: true };
  };
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < mcqs.length; i++) {
      const mcq = mcqs[i];
      // Prepare payload for backend (options as object, correctAnswer as label)
      const payload = {
        question: mcq.question,
        options: mcq.options,
      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4">
          <label className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded cursor-pointer">
            <input type="file" accept="application/json" className="hidden" onChange={handleFileChange} disabled={uploading} />
            Select File
          </label>
        ...
        {successMsg && <div className="mt-4 text-green-400 font-semibold">{successMsg}</div>}
        {errorMsg && <div className="mt-4 text-red-400 font-semibold">{errorMsg}</div>}
      </div>
      <div className="md:w-1/2 w-full bg-gray-900 rounded p-4 mt-8 md:mt-0">
        <h3 className="text-lg font-bold mb-2 text-green-400">JSON Format Example</h3>
        <pre className="bg-gray-800 text-gray-200 p-4 rounded overflow-x-auto text-sm">
{SAMPLE_JSON}
        </pre>
        <div className="mt-2 text-gray-400 text-xs">
          <div><b>Required fields:</b> question, options (A-D), correctAnswer (A-D)</div>
          <div><b>Optional fields:</b> explanation</div>
          <div><b>correctAnswer:</b> Must be one of 'A', 'B', 'C', 'D'</div>
        </div>
      </div>
    </div>
  );
